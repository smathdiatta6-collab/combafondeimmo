...
import { MapPin, ArrowRight, Bed, Bath, Pencil, Images } from 'lucide-react';
...
const PropertyCard: React.FC<PropertyCardProps> = ({ property }) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all group border border-gray-100 relative flex flex-col justify-between"
    >
      {isSuperAdmin && (
        <Link
      ...
      <div>
        <Link to={`/biens/${property.id}`} className="block relative aspect-[4/3] overflow-hidden">
          ...
          {photoCount > 1 && (
            <span className="absolute bottom-3 right-3 z-10 bg-black/65 backdrop-blur-md text-white text-[11px] font-bold px-2.5 py-1 rounded-lg flex items-center gap-1 shadow">
              <Images size={13} /> {photoCount} photos
            </span>
          )}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
            <span className="text-white font-medium flex items-center gap-2">
              Voir la galerie & détails <ArrowRight size={16} />
            </span>
          </div>
        </Link>
      ...
      <div className="p-6 pt-0">
        <div className="flex justify-between items-center pt-4 border-t border-gray-50">
          ...
          >
            Détails & Photos <ArrowRight size={14} />
          </Link>
        </div>
      </div>
    </motion.div>
